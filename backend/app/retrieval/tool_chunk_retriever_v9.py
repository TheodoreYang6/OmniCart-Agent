...
from app.core.config import QDRANT_URL, REDIS_CACHE_TTL_SEARCH, USE_QDRANT, V9_CHUNK_COLLECTION_NAME, V9_RERANK_TIMEOUT
...
def _filters(plan: Any, constraints: Any) -> dict[str, Any]:
    ...
    return {
        "category": value(constraints, "category") or _plan_value(plan, "category"),
...
def _explicit_terms(query: str, plan: Any) -> bool:
    ...
    return bool(
        terms or _ENTITY_RE.search(query) or re.search(r"(?:苹果|华为|小米|索尼|iphone|ipad|airpods)", query, re.I)
    )
...
def _qdrant_filter(filters: dict):
    ...
    if filters.get("sub_category"):
        ...
        must.append(
            FieldCondition(
                key="sub_category",
                match=(MatchAny(any=list(sub)) if isinstance(sub, (list, tuple, set)) else MatchValue(value=sub)),
            )
        )
    if filters.get("price_max") is not None or filters.get("price_min") is not None:
        must.append(
            FieldCondition(key="price", range=Range(gte=filters.get("price_min"), lte=filters.get("price_max")))
        )
...
class ToolChunkRetrieverV9:
    ...
    async def search(
        self, *, query: str, plan: Any, constraints: Any, intent: str = "recommend", top_k: int = 5
    ) -> dict[str, Any]:
    ...
    async def _search_once(
        self, query: str, plan: Any, constraints: Any, intent: str, top_k: int, signature: str
    ) -> dict[str, Any]:
        ...
        filter_result, all_evidence = await asyncio.gather(
            self._filterer.filter(
                query=query,
                plan={**plan_snapshot(plan, constraints), "intent": intent},
                constraints=constraints,
                candidates=reranked[:12],
            ),
            evidence_task,
        )
        final = self._apply_filter(reranked[:12], filter_result, top_k)
        evidence_pack = {p["product_id"]: all_evidence.get(p["product_id"], []) for p in final if p.get("product_id")}
        ...
        return {
            ...
            "reranked_count": len(reranked[:12]),
            "products": final,
            "filter": filter_result,
            "evidence_pack": evidence_pack,
            "latency_ms": elapsed,
            "source": "v9_lexical_dimension_fallback" if dimension_mismatch else "v9",
    ...
    def _query_chunks(self, vector: list[float], query: str, filters: dict, hybrid: bool) -> list[dict]:
                        ...
                        try:
                            ...
                            if sparse[0]:
                                result = client.query_points(
                                    ...
                                    prefetch=[
                                        Prefetch(query=vector, using="dense", limit=100, filter=qfilter),
                                        Prefetch(
        ...
        local_hits = self._local_hits(vector, filters)
        if local_hits:
            return local_hits

        # ``backend/data`` stores generated vector indexes and is intentionally
        # gitignored. Fresh clones and CI therefore have the JSON catalog but no
        # local embedding cache. The third degradation tier must remain useful:
        # fall back to deterministic lexical retrieval instead of returning an
        # empty candidate set and cascading that absence through every workflow.
        logger.info("v9 local vector cache unavailable or empty; using lexical fallback")
        return self._lexical_hits(query, filters)
                ...
                out.append(
                    {
                        ...
                        "score": sum(a * b for a, b in zip(vector, emb, strict=False)) / denom,
                        "payload": payload,
                        "chunk_type": payload.get("chunk_type", ""),
                    }
                )
    ...
    def _aggregate(self, hits: list[dict], filters: dict) -> list[dict]:
        ...
        for pid, product_hits in grouped.items():
            ...
            identity_support = max(
                per_type.get("identity", -1.0),
                per_type.get("facts", -1.0),
                per_type.get("marketing", -1.0),
                per_type.get("faq", -1.0),
            )
            ...
            candidates.append(
                {
                    ...
                    "chunk_aggregate_score": round(score, 5),
                    "chunk_type_support": sorted(per_type),
                }
            )
        ...
        evidence = [
            {
                ...
                "source_type": "chunk",
                "content": (h.get("payload") or {}).get("text", ""),
            }
            for p in candidates
            for h in (p.get("matched_chunks") or [])
        ]
        try:
            return (
                await asyncio.wait_for(
                    self._reranker.rerank(query=query, products=candidates, evidence=evidence),
                    timeout=V9_RERANK_TIMEOUT,
                )
            )[:12]
        except Exception as exc:
    ...
    def _evidence(product: dict) -> list[dict]:
        ...
        for hit in product.get("matched_chunks") or []:
            ...
            out.append(
                {
                    "evidence_id": f"v9:{payload.get('chunk_id', '')}",
                    "product_id": product.get("product_id"),
                    "source_type": payload.get("source_type", payload.get("chunk_type", "")),
                    "content": payload.get("text", "")[:320],
