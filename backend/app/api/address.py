...
async def list_addresses(user_id: str = DEMO_USER_ID, actor: Actor = Depends(resolve_actor)):
...
async def create_address(req: AddressCreate, user_id: str = DEMO_USER_ID,
    ...
    effective_uid = actor_or_legacy(actor, _uid(req.user_id or user_id), protected=True).user_id
...
async def delete_address(address_id: str, user_id: str = DEMO_USER_ID,
                         actor: Actor = Depends(resolve_actor)):
    repo = get_address_repo()
    effective_uid = actor_or_legacy(actor, user_id, protected=True).user_id
    ok = repo.delete(address_id, effective_uid)
    if not ok:
        raise HTTPException(status_code=404, detail="address not found")
    return {"ok": True}
