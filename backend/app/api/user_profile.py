...
async def parse_preview(req: ParseRequest, actor: Actor = Depends(resolve_actor)):
    """仅解析 raw_text，不写入数据库。供 Android 解析预览使用。"""
    user_id = require_user(actor).user_id
    if not req.raw_text.strip():
        return {"ok": False, "error": "raw_text required"}
    svc = get_user_profile_service()
    parsed = await svc.parse_only(user_id, req.raw_text.strip())
    if parsed is None:
...
async def save_entry(req: EntrySaveRequest, actor: Actor = Depends(resolve_actor)):
    """解析 raw_text 并保存为新条目（不合并旧条目）。entry_id 非空则覆盖。"""
    user_id = require_user(actor).user_id
    if not req.raw_text.strip():
        return {"ok": False, "error": "raw_text required"}
    svc = get_user_profile_service()
    entry = await svc.parse_and_save(user_id, req.raw_text.strip(), req.entry_id)
    if entry is None:
        return {"ok": False, "error": "解析失败，请检查输入内容是否包含明确品类"}
...
async def get_profile(user_id: str = Query(default=""), actor: Actor = Depends(resolve_actor)):
...
async def save_profile(req: EntrySaveRequest, actor: Actor = Depends(resolve_actor)):
...
async def reset_profile(user_id: str = Query(default=""), actor: Actor = Depends(resolve_actor)):
    """[已废弃] 删除用户全部偏好条目。"""
    user_id = require_user(actor).user_id
    svc = get_user_profile_service()
    entries = await svc.list_all_entries(user_id)
