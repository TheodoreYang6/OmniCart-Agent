...
def _product() -> Product:
    return Product(
        product_id="p-1", title="示例蓝牙耳机", brand="欧米", category="数码电子",
        sub_category="真无线耳机", base_price=399, image_path="2_数码电子/images/p-1.jpg", skus=[],
        rag_knowledge=RagKnowledge(
            marketing_description="舒适佩戴。" * 100,
            official_faq=[FaqItem(question="续航多久？", answer="充电盒可提供长续航。" * 60)],
            user_reviews=[ReviewItem(nickname="a", rating=5, content="佩戴舒适，通勤很好用。"),
...
def test_filter_rejects_out_of_scope_ids_and_budget():
    ...
    result = validate_filter_result({"primary": [{"product_id": "a", "reason": "ok"},
                                                {"product_id": "outside", "reason": "bad"},
                                                {"product_id": "b", "reason": "over"}],
                                    "alternative": [], "conditional": [], "exclude": []},
                                  candidates, {"budget_max": 300})
...
def test_filter_fallback_keeps_rerank_order():
    result = deterministic_filter([{"product_id": "a", "price": 99, "matched_chunks": []},
                                   {"product_id": "b", "price": 199, "matched_chunks": []}],
                                  {"budget_max": 300}, "timeout")
...
def test_explicit_missing_group_never_falls_back_to_irrelevant_cards():
    ...
    out = ToolChunkRetrieverV9._apply_filter(
        candidates,
        {"primary": [], "alternative": [], "conditional": [], "exclude": [],
         "missing_group": "缺少蓝牙耳机候选", "status": "model"},
        5,
    )
    assert out == []
...
async def test_tool_search_embeds_once_and_reuses_the_same_hit_snapshot():
    ...
    class Retriever(ToolChunkRetrieverV9):
        ...
        def _query_chunks(self, vector, query, filters, hybrid):
            ...
            return [{"product_id": product.product_id, "score": 0.9,
                     "chunk_type": "identity", "payload": hit_chunk.to_qdrant_payload()}]
...
def test_router_alias_subcategory_is_not_used_as_exact_vector_filter():
    ...
    assert retriever._validated_filters({"category": "数码电子", "sub_category": "真无线耳机"})["sub_category"] == "真无线耳机"
