...
class TestHybridSearch:
    ...
    def test_hybrid_with_stub_falls_back_to_text(self, text_retriever):
        """Stub 向量仓库 → hybrid_search 应降级为纯关键词搜索。"""
        results = text_retriever.hybrid_search("蓝牙耳机", top_k=5)
    ...
    def test_hybrid_returns_evidence_ids(self, text_retriever):
        """hybrid_search 结果应包含 evidence_ids。"""
        results = text_retriever.hybrid_search("精华", top_k=3)
        for r in results:
            assert "evidence_ids" in r
            assert len(r["evidence_ids"]) > 0

    def test_hybrid_respects_top_k(self, text_retriever):
        """结果数量不应超过 top_k。"""
        results = text_retriever.hybrid_search("咖啡", top_k=3)
        assert len(results) <= 3

    def test_hybrid_empty_query_returns_results(self, text_retriever):
        """空查询不应崩溃。"""
        results = text_retriever.hybrid_search("", top_k=5)
    ...
    def test_hybrid_search_format_matches_search(self, text_retriever):
        """hybrid_search 格式应与 search() 一致。"""
        text_results = text_retriever.search("防晒", top_k=3)
        hybrid_results = text_retriever.hybrid_search("防晒", top_k=3)
