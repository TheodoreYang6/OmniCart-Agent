...
**版本**: V7 `evidence_scoring_v1` | **日期**: 2026-06-05 | **状态**: 生产级 (Memory Lite)

**V7 更新**: Memory Lite 收敛 — 移除 MemoryRetriever/PreferenceBonus/AvoidPenalty/EvidenceMetrics；RRF删除；语义相关度接入余弦相似度校准；品类安全校验；must_tags软约束化。
...
OmniCart V7 评分系统是一个**去 LLM 化、规则驱动**的多维商品评分引擎。

核心设计原则:
- **当前约束优先**: 用户刚说的和当前会话累积约束 > 一切
- **可复算**: 所有评分参数均可从检索结果和商品数据确定性计算
- **语义相关度**: Qwen Embedding + Qdrant 余弦相似度校准到商业可读区间
- **品类安全**: LLM 输出的品类必须在校验白名单中
