...
def _ensure(client, name: str, recreate: bool):
    ...
    try:
        ...
        client.create_collection(name, vectors_config={"dense": VectorParams(size=EMBEDDING_DIMENSION, distance=Distance.COSINE)},
                                 sparse_vectors_config={"bm25": SparseVectorParams(index=SparseIndexParams())})
    for field, schema in {"product_id": PayloadSchemaType.KEYWORD, "category": PayloadSchemaType.KEYWORD,
                          "sub_category": PayloadSchemaType.KEYWORD, "brand": PayloadSchemaType.KEYWORD,
                          "price": PayloadSchemaType.FLOAT, "source_type": PayloadSchemaType.KEYWORD,
                          "fact_keys": PayloadSchemaType.KEYWORD}.items():
        try:
    ...
    try:
        ...
        for collection, docs in ((discovery, discovery_docs), (evidence, evidence_docs)):
            ...
            for d, vector in zip(docs, vectors, strict=True):
                ...
                points.append(PointStruct(id=str(uuid.uuid5(uuid.NAMESPACE_URL, identity)),
                                         vector=point_vector, payload=d.payload))
